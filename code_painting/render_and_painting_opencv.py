...
class RobotRenderer:
    ...
    def _load_robot(self):
        ...
        try:
            ...
            # "/home/pine/RoboTwin2/assets/embodiments/aloha-agilex/urdf/arx5_description_isaac.urdf"
    ...
    def _opencv_to_sapien_pose(self, opencv_position: np.ndarray, opencv_rotation: np.ndarray) -> sapien.Pose:
        ...
        # 注意：当前实现中坐标系转换矩阵为单位矩阵，表示没有进行坐标系转换
        # 如果需要正确的坐标系转换，应该使用以下矩阵：
    ...
    def set_arm_poses(self, 
        ...
        try:
            ...
            # 使用机器人的路径规划功能
            print("########### LEFT: ##############")
            # print("left_target_pose=",left_target_pose)
            ...
            # print("[0.0] * len(right_target_pose)",([0.0] * len(right_target_pose)))
            # self.robot.set_arm_joints(
            #     # target_position=right_target_joints,
            #     target_position=right_target_pose,
            #     target_velocity=[0.0] * len(right_target_pose),
    ...
    def render_and_save(self, head_path: str):
        ...
        # 图像翻转：flipCode=0 表示垂直翻转，1 表示水平翻转，-1 表示水平+垂直翻转
        head_img = cv2.flip(head_img, -1)
                  
        # 现在不需要翻转图像，因为相机坐标系转换已经处理了
...
def demo_usage():
    ...
    # xy_to_yx = np.array([
    #     [0,  1,  0],  # X轴保持不变
    #     [1,  0,  0],  # Y轴翻转：下 -> 上 (简单朝左偏了)
    #     [0,  0,  1]   # Z轴翻转：前 -> 后
    # ])   
    left_pos_world = left_pos_world @ xy_to_yx
    right_pos_world = right_pos_world @ xy_to_yx
    print(f"世界系左腕旋转后位置: {left_pos_world}")
    print(f"世界系右腕旋转后位置: {right_pos_world}")
    
    # left_pos_world[0] += 0.5
    # right_pos_world[0] += 0.3
    
    # # left_pos_world[1] += 0.15  # 1.2
    # # right_pos_world[1] -= 0.15      
    # left_pos_world[1] += 0.2 # 1.2
    # right_pos_world[1] += 0.2
              
    left_pos_world[2] += 1.3
    right_pos_world[2] += 1.3
