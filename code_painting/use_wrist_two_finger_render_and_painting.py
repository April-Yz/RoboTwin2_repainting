...
def generate_robot_video(json_path: str, 
    ...
    try:
        ...
        for frame_idx in range(actual_frames):
            ...
            # 计算左手抓取姿态
            left_p = 0.5 * (left_thumb_tip + left_index_tip)  # 位置 P
            left_z = left_index_tip - left_thumb_tip
            left_z_hat = left_z / (np.linalg.norm(left_z) + 1e-9)
            
            left_v = left_p - left_wrist
            left_x_tmp = left_v - np.dot(left_v, left_z_hat) * left_z_hat  # 投影到与 z 垂直的平面
            if np.linalg.norm(left_x_tmp) < 1e-6:
                # 退化保护：比如用全局上方向 up 做参考
                up = np.array([0, 0, 1.0])
                left_x_tmp = up - np.dot(up, left_z_hat) * left_z_hat
            
            left_x_hat = left_x_tmp / (np.linalg.norm(left_x_tmp) + 1e-9)
            left_y_hat = np.cross(left_z_hat, left_x_hat)
            
            left_rot_world = np.column_stack([left_x_hat, left_y_hat, left_z_hat])  # 抓取姿态
            left_pos_world = left_p  # 使用拇指和食指的中点作为位置
            
            # 右手
            right_thumb_tip = np.array(data["right"]["thumbTip"]["position"][frame_idx], dtype=float)
            right_index_tip = np.array(data["right"]["indexFingerTip"]["position"][frame_idx], dtype=float)
            right_wrist = np.array(data["right"]["wrists"]["position"][frame_idx], dtype=float)
            
            # 计算右手抓取姿态
            right_p = 0.5 * (right_thumb_tip + right_index_tip)  # 位置 P
            right_z = right_index_tip - right_thumb_tip
            right_z_hat = right_z / (np.linalg.norm(right_z) + 1e-9)
            
            right_v = right_p - right_wrist
            right_x_tmp = right_v - np.dot(right_v, right_z_hat) * right_z_hat  # 投影到与 z 垂直的平面
            if np.linalg.norm(right_x_tmp) < 1e-6:
                # 退化保护：比如用全局上方向 up 做参考
                up = np.array([0, 0, 1.0])
                right_x_tmp = up - np.dot(up, right_z_hat) * right_z_hat
            
            right_x_hat = right_x_tmp / (np.linalg.norm(right_x_tmp) + 1e-9)
            right_y_hat = np.cross(right_z_hat, right_x_hat)
            
            right_rot_world = np.column_stack([right_x_hat, right_y_hat, right_z_hat])  # 抓取姿态
            right_pos_world = right_p  # 使用拇指和食指的中点作为位置
...
def demo_video_generation():
    ...
    num_frames = 399 # 299
    task_name = "basic_pick_place" #"add_remove_lid"   # "assemble_disassemble_furniture_bench_lamp"  #"clean_surface" # "clean_cups"
